 ...
 * @width: used to get width.
 * @buffer: Buffer array to handle print
 * @size: Used to specify size 
 * write_number - Prints a string 
 * @precision: Used to specify precision
 ...
 * @is_negative: Number indicating if there is a negative num
	...
	int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* Add an additional char to the left of buffer [buffer>padd]*/
		{
			return (write(1, &buffer[ind], length) + write(1, &buffer[0], i));
		}
		else /* Assign an additional char to the left of padding [padd>buffer]*/
		{
...
/**
 * write_pointer – used to record a memory address
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		for (i = 3; i < width - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* Assign additional char to the left of buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
